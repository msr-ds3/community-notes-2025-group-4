...
```{r data-cleaning}
notes <- notes |> 
    mutate(datetime = as.POSIXct(createdAtMillis / 1000, origin = "1970-01-01", tz = "UTC")) #convert to a datetime format from milliseconds
notes <- notes |> 
...
ccdf_ratio <- inner_join(notes, ratings_2, by="noteId") |> 
    ...
    mutate(rank=min_rank(ratio_helpful), n=n(), ccdf=(1-((rank-1)/n)) *100) |>
