...
ratio <- joined_ratings_notes |> group_by(noteId) |> summarize(helpful_count = sum(helpful),total_votes = n()) |> select(noteId, is_misleading, helpful_count, unhelpful_count, total_votes)
ratio <- ratio |> mutate(ratio_helpful = helpful_count/total_votes) |> arrange(ratio_helpful)
ratio_ccdf <- ratio |>  mutate(cdf_fraction = cumsum(ratio_helpful)) |> mutate(ccdf_fraction = (1- cdf_fraction) * 100) |> filter(ccdf_fraction > 0)
